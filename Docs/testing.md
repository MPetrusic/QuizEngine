...
QE-6 tests use `FakeTransport` raw-payload events and `MultiplayerWireCodec`, not app networking. Cover exact content/protocol/capability handshake, unsupported legacy transport, wrong sender, oversized or malformed bytes, replay IDs, duplicate sequence numbers, stale match IDs, future rounds followed by valid out-of-order rounds, duplicate configuration retries, and first-terminal-wins lifecycle races. Pass a fake persistence store to a progress manager and verify that a match receipt survives reload and prevents duplicate statistics/rewards.
